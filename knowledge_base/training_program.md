...
网页默认始终是规则 Teacher。神经网络仍全部是显式实验候选：独立 80 个经典物理牌墙、四座轮换中，run3 对
Teacher 为 **+1.34 ± 1.38** 分/局（95% CI **[−1.36, +4.04]**），方向为正但不够晋升；从 run3 继续
DAgger 微调的 run4 为 **−2.38 ± 1.13**（**[−4.59, −0.17]**），且同墙配对比 run3 **−3.72 ± 1.57**
（**[−6.79, −0.64]**），已淘汰为部署/训练起点。后续不得再把“更高的混合轨迹动作一致率”当作选择
checkpoint 的依据；新候选必须先在未参与调参的牌墙上通过正向下界。
