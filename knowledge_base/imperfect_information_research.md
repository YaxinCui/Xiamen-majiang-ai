...
## 2026-08-07：初始副露的精确密度条件 proposal（仅审计）

为先验证“约束公开动作”能否在**不伪造 `p/q`** 的前提下提升粒子覆盖，新增
`core_initial_response_claim_exact_density_v0`。它只接受最窄的 core 前缀：本家在开局摸牌后立即弃牌，随后
另一家立即吃／碰／明杠；该家在此之前没有摸牌，因此公开副露消耗的 2／3 张牌恰好是其初始暗手的约束。

对未知物理牌池的目标初始暗手 `H`，proposal 直接按多元超几何分布抽样
`q(H)=p(H | H 包含公开副露所需牌)`，而不是从任意暗手抽样后交换牌。动态规划精确计数带重复物理牌的组合数，
并返回 `P(H 包含所需牌)`，故这一 setup proposal 的重要性修正严格为
`p/q=P(H 包含所需牌)`。其他暗手与牌墙在该条件下仍均匀分配；世界、牌墙、牌面和随机数均不导出。

固定 core `seed=2`、首个吃牌前缀、256 粒子审计中，普通 setup prior 重放接受 **40/256**；精确条件 proposal
接受 **256/256**，结构条件概率（亦即 `p/q`）为 **0.13146149**。在冻结 Teacher 的平滑行为似然下，接受粒子的
ESS 为 **190.04/256 = 0.742**。这只说明该**一个**可枚举的 setup 约束和密度公式实现正确；不覆盖后续摸牌、
