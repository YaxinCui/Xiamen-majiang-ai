#!/usr/bin/env python3
"""Train the fixed CPU-small-MLP residual from confirmed response reviews."""
...
FRESH_SEED = 202633000
HIDDEN_SIZE = 128
EPOCHS = 12
HUMAN_RESPONSE_REVIEW_WEIGHT = 4.0
HUMAN_DISAGREEMENT_WEIGHT = 2.0
MINIMUM_TRAIN_VALIDATION_LABELS = 240
MINIMUM_TRAIN_VALIDATION_DISAGREEMENTS = 35
MINIMUM_TRAIN_VALIDATION_GROUPS = 80
MINIMUM_SELECTION_DECISIONS = 20


def _review_path(directory: Path, split: str) -> Path:
    return directory / f"{split}.response-review.jsonl"
...
def validate_inputs(review_split_dir: Path) -> dict[str, object]:
    teacher_paths = {
        split: _teacher_path(split) for split in ("train", "validation", "test")
    }
    review_paths = {
        split: _review_path(review_split_dir, split)
    ...
    if missing:
        raise ValueError("固定 response review 训练输入缺失：" + ", ".join(missing))

    # The test file is only stat'ed above.  Its bytes remain physically unread
    # until a validation-selected checkpoint and margin have been frozen.
    train_records = read_response_review_labels(review_paths["train"])
    validation_records = read_response_review_labels(review_paths["validation"])
    audit = audit_response_review_labels(
        [*train_records, *validation_records],
        minimum_confirmed_labels=MINIMUM_TRAIN_VALIDATION_LABELS,
        minimum_confirmed_disagreements=(
            MINIMUM_TRAIN_VALIDATION_DISAGREEMENTS
        ),
        minimum_groups=MINIMUM_TRAIN_VALIDATION_GROUPS,
    )
    if not audit["ready_for_manual_training_review"]:
        raise ValueError(
            "response review train/validation 未通过 fixed v1 门槛："
            + ", ".join(audit["gate_reasons"])
        )
    train_rows = read_confirmed_response_review_decisions(review_paths["train"])
    validation_rows = read_confirmed_response_review_decisions(
        review_paths["validation"]
    )
    train_groups = {group for group, _decision in train_rows}
    validation_groups = {group for group, _decision in validation_rows}
    if train_groups & validation_groups:
        raise ValueError("response review train/validation 存在物理牌局 group 重叠")
    return {
...
def build_training_command(
    ...
    return [
        ...
        "--human-response-review-train", str(review_paths["train"]),
        "--human-response-review-validation", str(review_paths["validation"]),
        "--allow-local-human-response-review-data",
        "--human-response-review-weight", str(HUMAN_RESPONSE_REVIEW_WEIGHT),
        ...
        "--checkpoint-selection-source", "local_human_response_review_opt_in",
...
def _safe_summary(
    validated: dict[str, object], output_dir: Path
) -> dict[str, object]:
    audit = validated["aggregate_audit"]
    assert isinstance(audit, dict)
    return {
        "status": "fixed_human_response_review_residual_v1_inputs_ready",
        "protocol": {
            "architecture": "candidate_mlp",
            "feature_version": 3,
            ...
            "human_response_review_weight": HUMAN_RESPONSE_REVIEW_WEIGHT,
        ...
        "review_train_validation": {
            ...
            "action_counts": audit["confirmed_action_counts"],
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/human-response-review-residual-v1"),
    )
    ...
    if args.output_dir.exists() and (
        ...
        raise ValueError("fixed response review v1 输出已存在且非空，拒绝覆盖或续训")
    subprocess.run(
        build_training_command(
            validated, output_dir=args.output_dir, device=args.device
        ),
        cwd=ROOT,
        check=True,
    )
