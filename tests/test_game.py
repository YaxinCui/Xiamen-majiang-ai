...
from xiamen_mahjong.tiles import BASE_TILE_COUNT, WHITE_DRAGON, base_wall


class GameTests(unittest.TestCase):
    def test_new120_wall_contains_only_suits_white_and_flowers(self):
    ...
    def test_new120_deals_sixteen_tiles_and_excludes_big_honors(self):
        game = XiamenMahjongGame(seed=1, rules=XiamenRules.new120(), dealer=0)
        physical_tiles = [
            tile
            ...
            for tile in [
                ...
                *player.flowers,
                *player.discards,
                *(tile for meld in player.melds for tile in meld["tiles"]),
            ]
    ...
    def test_new120_double_gold_cannot_win_as_an_ordinary_hand(self):
        game = XiamenMahjongGame(seed=20260815, rules=XiamenRules.new120())
        game.gold_tile = 8
        game.players[0].hand = [
    ...
    def test_new120_forces_available_tour_upgrade(self):
        game = XiamenMahjongGame(seed=20260816, rules=XiamenRules.new120())
        game.gold_tile = 8
        ...
        self.assertEqual(
            {action["kind"] for action in game._turn_actions(0)},
            {"advance_tour"},
        )

    def test_new120_never_turns_a_later_third_gold_into_three_gold_down(self):
        game = XiamenMahjongGame(seed=1, rules=XiamenRules.new120(), dealer=0)
        game.gold_tile = 8
        game.phase = "discard"
    ...
    def test_new120_fixed_scoring_uses_main_score_plus_water(self):
        game = XiamenMahjongGame(seed=20260814, rules=XiamenRules.new120())
        game.gold_tile = 8
        game.dealer = 0
        for player in game.players:
            player.score = 0
    ...
    def test_new120_can_run_until_settlement(self):
        game = XiamenMahjongGame(seed=120, rules=XiamenRules.new120())
        steps = 0
